...
# UPDATED:  260729 BY: Claude(Sonnet5)::WIZ-00.Copilot::pickaxe.SOLOMON - LB-03 fix (instruction-rollup data loss), 121 tests green
# UPDATED: 260729 BY: Claude(Sonnet5)::WIZ-00.Copilot::pickaxe.SOLOMON - LB-04 fix (.github/instructions/ dest routing), 123 tests green
...
**Status (reconciled 2026-07-29):** v0.4.2 shipped — `discover` (incl. `commit-trends`, `drift`, `--submodules-only`), `diagnose` (incl. `instruction-bloat`), `deliver` (`instruction-rollup`, first-ever verb), `scan` (incl. `already_extracted`), `backup`, `restore` — 123 tests green. Extraction pipeline (Track A, `--execute` mode) remains **0% built** — no pipeline scaffolding exists yet; see ROADMAP.md Track A checklist.

## F-03: Repo-hygiene diagnostics — instruction bloat + rollup (shipped 2026-07-29, v0.4.0; bugfix v0.4.1)
...
- `deliver instruction-rollup` — the **first-ever `deliver` verb to ship** (A2); extracts a flagged block into a new scoped instruction file using the existing Instruction Inheritance Pattern frontmatter, dry-run by default, `--execute` to write, idempotent (A4). **v0.4.1:** fixed LB-03 — a whole-file finding co-occurring with section findings on the same source caused every extraction after the first to silently write an empty, frontmatter-only file. Findings are now snapshotted per source file and overlapping section findings are reported `skipped_overlap` instead of double-extracted.

19 new tests (`TestDiagnoseNounDispatch`, `TestDiagnoseInstructionBloat`, `TestDeliverInstructionRollup`) + 2 LB-03 regression tests, all green — 121/121 total.
